...
def test_extract_valid_certs(mocker, monkeypatch, tmp_path):
    ...
    mock_named_temp_file = mocker.patch("app.common.tls.tempfile.NamedTemporaryFile")
...
def test_extract_invalid_base64_cert(monkeypatch):
...
def test_extract_no_truststore_vars(monkeypatch):
...
def test_load_valid_certs(mocker):
...
def test_load_certs_error(mocker):
    mock_create_context = mocker.patch("app.common.tls.ssl.create_default_context")
    mock_ctx = mocker.MagicMock()
    mock_create_context.return_value = mock_ctx
